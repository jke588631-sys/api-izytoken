...
func HandleStreamFinalResponse(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo) {
	...
	if claudeInfo.Usage.CompletionTokens == 0 || !claudeInfo.Done {
		...
		// Skip fallback estimation when upstream explicitly reported zero output tokens.
		// This preserves CompletionTokens=0 so the billing layer can detect
		// "no output" and refund instead of charging for a useless request.
		upstreamReportedZeroOutput := claudeInfo.Usage.OutputTokens == 0 && claudeInfo.Usage.CompletionTokens == 0
		if !upstreamReportedZeroOutput {
