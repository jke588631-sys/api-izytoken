...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	// Treat as error when upstream consumed input tokens but returned zero output.
	// The user received nothing useful — refund pre-consumed quota and log as error.
	// Check both CompletionTokens (may be fallback-estimated) and OutputTokens (raw upstream value).
	if usage != nil && usage.CompletionTokens == 0 && usage.OutputTokens == 0 && usage.PromptTokens > 0 {
		...
		if constant.ErrorLogEnabled {
			...
			useTimeSec := 0
			if !startTime.IsZero() {
				useTimeSec = int(time.Since(startTime).Seconds())
			}
			errMsg := fmt.Sprintf("no output returned (prompt_tokens=%d, model=%s, stream=%s)",
				usage.PromptTokens, relayInfo.OriginModelName, streamSummary)
			other := map[string]interface{}{
				"error_type":    "no_output",
				"stream_status": streamSummary,
				"prompt_tokens": usage.PromptTokens,
			}
			model.RecordErrorLog(ctx, relayInfo.UserId, relayInfo.ChannelId,
				relayInfo.OriginModelName, ctx.GetString("token_name"), errMsg,
				relayInfo.TokenId, useTimeSec,
				relayInfo.IsStream, relayInfo.UsingGroup, other)
		}
		logger.LogWarn(ctx, fmt.Sprintf("no output, refunded pre-consumed quota for user %d, model %s, stream=%s",
			relayInfo.UserId, relayInfo.OriginModelName, streamSummary))
		return
	}
